#!/bin/sh
# Health check script embedded in the Python app container.
...
Embedded Docker health check for the Python app container.
Runs two checks in sequence:
  1. /health  — confirms the Flask process is alive (HTTP 200 expected)
  2. /ready   — confirms the app + MySQL DB are reachable (HTTP 200 expected)
...
Environment (read from container env):
  PORT  Port the app is listening on (default: 5000)
EOF
  exit 0
fi
