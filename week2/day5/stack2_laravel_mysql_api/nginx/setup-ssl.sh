...
# Script: setup-ssl.sh
# Description: Generate a local CA-signed SSL certificate for stack2.devops.local.
...
CA_KEY="$KEY_DIR/stack2-ca.key"
CA_CERT="$CERT_DIR/stack2-ca.crt"
SERVER_KEY="$KEY_DIR/stack2.key"
SERVER_CSR="/tmp/stack2.csr"
SERVER_CERT="$CERT_DIR/stack2.crt"
...
if [[ ! -f "$CA_KEY" ]]; then
    info "Step 1/5: Generating CA private key..."
    openssl genrsa -out "$CA_KEY" 4096 2>/dev/null
    chmod 600 "$CA_KEY"
    pass "CA private key created"
else
    info "Step 1/5: CA private key already exists — skipping"
fi

# Step 2: CA certificate (self-signed, valid 10 years)
if [[ ! -f "$CA_CERT" ]]; then
    info "Step 2/5: Generating CA certificate..."
    openssl req -new -x509 -key "$CA_KEY" -sha256 -days 3650 \
        -out "$CA_CERT" \
        -subj "/C=IN/ST=DevOps/L=Lab/O=DevOps Bootcamp/OU=Stack2 CA/CN=Stack2 Local CA" \
        2>/dev/null
    pass "CA certificate created (10 year validity)"
else
    info "Step 2/5: CA certificate already exists — skipping"
fi

# Step 3: Server private key
info "Step 3/5: Generating server private key..."
openssl genrsa -out "$SERVER_KEY" 2048 2>/dev/null
chmod 600 "$SERVER_KEY"
pass "Server private key created"

# Step 4: CSR + sign with CA (SAN support)
info "Step 4/5: Creating CSR and signing with CA..."
cat > /tmp/stack2-san.cnf <<EOF
[req]
default_bits       = 2048
prompt             = no
...
openssl req -new -key "$SERVER_KEY" \
    -out "$SERVER_CSR" \
    -config /tmp/stack2-san.cnf 2>/dev/null

openssl x509 -req -in "$SERVER_CSR" \
    -CA "$CA_CERT" -CAkey "$CA_KEY" -CAcreateserial \
    -out "$SERVER_CERT" -days 825 -sha256 \
    -extfile /tmp/stack2-san.cnf -extensions v3_ext 2>/dev/null

rm -f "$SERVER_CSR" /tmp/stack2-san.cnf
pass "Server certificate signed by local CA (825 days)"

# Step 5: Install CA cert to system trust store (optional, for curl/wget)
info "Step 5/6: Installing CA to system trust store..."
if [[ -d /usr/local/share/ca-certificates ]]; then
    cp "$CA_CERT" /usr/local/share/ca-certificates/stack2-ca.crt
    update-ca-certificates 2>/dev/null || true
    pass "CA installed to system trust store"
elif [[ -d /etc/pki/ca-trust/source/anchors ]]; then
    cp "$CA_CERT" /etc/pki/ca-trust/source/anchors/stack2-ca.crt
...
if command -v certutil &>/dev/null && [[ -d "$NSSDB_DIR" ]]; then
    certutil -d "sql:$NSSDB_DIR" -D -n "Stack2 Local CA" 2>/dev/null || true
    certutil -d "sql:$NSSDB_DIR" -A -t "C,," -n "Stack2 Local CA" -i "$CA_CERT"
